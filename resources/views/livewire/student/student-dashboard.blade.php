<div class="space-y-6 font-sans">
    <!-- 1. Hero Learner Profile & Academic Standing Banner -->
                ...
                <div class="space-y-1">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-white/20 backdrop-blur-md text-white border border-white/30">
                            Student ID: {{ $student?->student_number ?? auth()->user()->login_id }}
                        </span>
                    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5 text-xs">
        ...
        <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-1">
            ...
            <span class="text-[10px] text-zinc-500 block">
                {{ $kpi['totalSubjects'] > 0 && $kpi['passedCount'] === $kpi['totalSubjects'] ? '100% Passed (≥ 75.00)' : 'Passing Status' }}
            </span>
        </div>

        <!-- 3. Subject Excellence Awards -->
        <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-1">
            <div class="flex items-center justify-between text-zinc-400">
        ...
        <div class="col-span-2 sm:col-span-1 bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-1">
            ...
            <span class="text-[10px] text-zinc-500 block">
                {{ ($kpi['lowestGrade'] ?? 0) >= 85 ? 'Honors Cap Met (≥ 85)' : 'Passing Threshold: 75' }}
            </span>
        </div>
    </div>

    <!-- 3. Visual Analytics Section (2 ApexCharts) -->
            ...
            <div class="border-b border-zinc-100 dark:border-zinc-800 pb-3 flex items-center justify-between">
                <div>
                    <h3 class="text-xs font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider flex items-center gap-2">
                        ...
                        <span>Quarterly Performance Trend (Q1–Q3)</span>
                    </h3>
                    <p class="text-[11px] text-zinc-500">
                        Quarter-by-quarter comparison of your General Average against Section Class Average
                    </p>
                </div>
            ...
            @if ($progressionChart['hasData'])
                <div wire:ignore
                    ...
                    x-init="(function() {
                        ...
                        var myAverages = @js($progressionChart['myAverages']);
                        var sectionAverages = @js($progressionChart['sectionAverages']);

                        function draw() {
                            var el = $el.querySelector('[data-chart-student-progression]');
                            ...
                            el._apexChart = new ApexCharts(el, {
                                ...
                                series: [
                                    { name: 'My Average', data: myAverages },
                    ...
                    <div data-chart-student-progression class="w-full h-64 min-h-[250px]"></div>
                </div>
            @else
        ...
        <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-4 flex flex-col justify-between">
            <div class="border-b border-zinc-100 dark:border-zinc-800 pb-3 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[11px] text-zinc-500">
                        Subject marks across core JHS learning areas (Passing Standard: 75.00)
                    </p>
                </div>
            ...
            @if ($subjectMasteryChart['hasData'])
                <div wire:ignore
                    ...
                    x-init="(function() {
                        ...
                        function draw() {
                            var el = $el.querySelector('[data-chart-student-subjects]');
                            if (!el || typeof ApexCharts === 'undefined') { setTimeout(draw, 80); return; }
                            if (el._apexChart) { try { el._apexChart.destroy(); } catch(e){} }
                            var dark = document.documentElement.classList.contains('dark');
                            var tc = dark ? '#a1a1aa' : '#52525b';
                    ...
                    <div data-chart-student-subjects class="w-full h-64 min-h-[250px]"></div>
                </div>
            @else
                <div class="h-64 flex flex-col items-center justify-center text-center p-6 text-zinc-400 bg-zinc-50 dark:bg-zinc-800/40 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-700">
                    <i class="fas fa-chart-bar text-2xl mb-2 text-zinc-300"></i>
                    <span class="text-xs font-medium">Subject mastery ratings will appear once grade entries are recorded.</span>
                </div>
            @endif
        </div>
    </div>

    <!-- 4. Official Learning Areas & Subject Grades Sheet (Q1 to Q3) -->
            ...
            <table class="w-full text-xs text-start">
                <thead class="bg-zinc-50 dark:bg-zinc-800/80 text-zinc-500 text-[10px] font-bold uppercase tracking-wider border-b border-zinc-200 dark:border-zinc-800">
                    <tr>
                        ...
                        <th class="py-3 px-4 text-start">Teacher Instructor</th>
                        @foreach ($quarters as $q)
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800 font-medium">
                    @forelse ($subjectRows as $idx => $row)
                        <tr class="hover:bg-zinc-50/80 dark:hover:bg-zinc-800/40 transition">
                            ...
                            <td class="py-3 px-4">
                                @if ($row['is_best'])
                                    <span class="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 inline-flex items-center gap-1 shadow-xs">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($quarters) + 5 }}" class="p-10 text-center text-zinc-400 italic">
                                No subject enrollments available for this school year.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <!-- 5. My Honors, Achievers & Recognitions Wall -->
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-6 space-y-4">
            <div class="border-b border-zinc-100 dark:border-zinc-800 pb-3 flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                        ...
                        <span>My Academic Honors & Awards Showcase</span>
                    </h3>
                    <p class="text-xs text-zinc-500">
                        Official recognitions earned based on DepEd Order No. 36, s. 2016 guidelines
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
                @foreach ($earnedAwards as $aw)
                    <div class="p-4 rounded-xl border border-amber-200 dark:border-amber-800/80 bg-gradient-to-br from-amber-50/60 to-white dark:from-amber-950/20 dark:to-zinc-900 space-y-2 shadow-xs flex items-start gap-3">
                        ...
                        <div class="space-y-0.5">
                            <span class="text-[9px] uppercase font-bold text-amber-700 dark:text-amber-400 block tracking-wider">
                                {{ $aw['type'] === 'honor' ? 'General Academic Honor' : ($aw['type'] === 'subject' ? 'Subject Excellence' : 'Special Citation') }}
                            </span>
