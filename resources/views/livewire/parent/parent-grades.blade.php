<div class="space-y-6 font-sans">
    ...
    @if ($children->count() > 1)
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4 shadow-sm flex items-center justify-between">
            <span class="text-xs font-bold text-zinc-500">Select Child for Report Card:</span>
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-6 space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-100 dark:border-zinc-800 pb-4">
                <div>
                    ...
                    <p class="text-xs text-zinc-500 mt-0.5">
                        Consolidated Quarterly Grades for {{ $selectedStudent->full_name }} (1st Quarter to 3rd Quarter).
                    </p>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <div class="p-3 bg-zinc-50 dark:bg-zinc-800/60 rounded-xl border border-zinc-100 dark:border-zinc-700/60">
                    ...
                    <span class="font-bold text-zinc-900 dark:text-zinc-100 text-sm block truncate">{{ $selectedStudent->full_name }}</span>
                </div>
                <div class="p-3 bg-zinc-50 dark:bg-zinc-800/60 rounded-xl border border-zinc-100 dark:border-zinc-700/60">
                    <span class="text-[10px] uppercase font-bold text-zinc-400 block">Student LRN / ID</span>
                    <span class="font-mono font-bold text-[#166534] dark:text-emerald-400 text-sm block">{{ $selectedStudent->student_number }}</span>
                </div>
                <div class="p-3 bg-zinc-50 dark:bg-zinc-800/60 rounded-xl border border-zinc-100 dark:border-zinc-700/60">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ count($quarters) + 5 }}" class="p-8 text-center text-zinc-400 italic">
                                    No subject records available for this child.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @else
        <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-dashed border-zinc-300 dark:border-zinc-700 p-12 text-center text-zinc-500">
            No linked child record found to view report card.
        </div>
    @endif
</div>
