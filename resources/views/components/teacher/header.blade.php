@props(['title' => 'Faculty Portal', 'subtitle' => 'Academic Performance Monitoring System'])
...
<header class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-6 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
    <div class="flex items-center gap-4">
        ...
        <button id="teacher-mobile-toggle" class="lg:hidden p-2 text-zinc-600 hover:text-zinc-900 rounded-lg hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800">
    ...
    <div class="flex items-center gap-3 sm:gap-4 text-xs font-medium">
        <!-- Active School Year & Quarter Indicator -->
        @php
            ...
            $advisedSection = $teacher && $activeSy ? $teacher->advisedSections()->where('school_year_id', $activeSy->id)->with(['section', 'yearLevel'])->first() : null;
        @endphp
        ...
        <div class="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#DCFCE7] text-[#166534] border border-emerald-200 font-bold">
            ...
            <span>SY {{ $activeSy ? $activeSy->school_year : 'None' }}</span>
            @if ($currentQuarter)
        ...
        @if ($advisedSection)
            <div class="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 font-bold text-[11px]">
                <i class="fas fa-shield-halved text-amber-600 dark:text-amber-400"></i>
                <span>Adviser: {{ $advisedSection->yearLevel?->name }} - {{ $advisedSection->section?->name }}</span>
            </div>
        @endif
