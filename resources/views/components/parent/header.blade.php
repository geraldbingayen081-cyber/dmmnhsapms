@props(['title' => 'Parent & Guardian Portal', 'subtitle' => 'Academic Performance Monitoring System'])
...
<header class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-6 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
    <div class="flex items-center gap-4">
        ...
        <button id="parent-mobile-toggle" class="lg:hidden p-2 text-zinc-600 hover:text-zinc-900 rounded-lg hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800">
    ...
    <div class="flex items-center gap-3 sm:gap-4 text-xs font-medium">
        ...
        @php
            ...
            $childrenCount = $parentModel ? $parentModel->students()->count() : 0;
        @endphp
        ...
        <div class="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 font-bold text-[11px]">
            <i class="fas fa-hands-holding-child text-amber-600 dark:text-amber-400"></i>
            <span>{{ $childrenCount }} {{ $childrenCount === 1 ? 'Child' : 'Children' }} Linked</span>
        </div>
