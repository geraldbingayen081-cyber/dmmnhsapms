...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    ...
    <body class="h-full bg-zinc-50 dark:bg-zinc-950 font-sans text-zinc-900 dark:text-zinc-100 antialiased">
        <div class="min-h-screen flex">
            ...
            <div id="parent-mobile-drawer" class="fixed inset-0 z-50 lg:hidden hidden">
                <div id="parent-drawer-backdrop" class="fixed inset-0 bg-zinc-950/60 backdrop-blur-sm transition-opacity"></div>
                <div class="fixed inset-y-0 left-0 max-w-xs w-full bg-[#1F2937] shadow-2xl flex flex-col z-10">
                    <div class="p-4 bg-[#166534] text-white flex items-center justify-between">
                        ...
                        <button id="parent-drawer-close" class="p-1 text-white hover:text-emerald-200">
            ...
            <div class="flex-1 flex flex-col min-w-0">
                <x-parent.header :title="$title ?? 'Parent & Guardian Portal'" :subtitle="$subtitle ?? 'Academic Performance Monitoring System'" />
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const toggleBtn = document.getElementById('parent-mobile-toggle');
                const closeBtn = document.getElementById('parent-drawer-close');
                const backdrop = document.getElementById('parent-drawer-backdrop');
                const drawer = document.getElementById('parent-mobile-drawer');
